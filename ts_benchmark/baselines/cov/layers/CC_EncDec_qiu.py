...
class CovCausalityEncoder(nn.Module):
    ...
    def forward(self, x, exog_future, use_exog=True):
        ...
        exog_history = self.sample_denorm(exog_history, x_history_means.repeat(1, 1, self.exog_dim), x_history_stdev.repeat(1, 1, self.exog_dim))
        exog_future = self.sample_denorm(exog_future, x_history_means.repeat(1, 1, self.exog_dim), x_history_stdev.repeat(1, 1, self.exog_dim))
