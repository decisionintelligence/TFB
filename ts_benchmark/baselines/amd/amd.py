...
from ts_benchmark.baselines.amd.models.tsAMD import Model
from ts_benchmark.baselines.amd.utils.tools import (
    EarlyStopping,
    ...
    def validate(
        ...
        with torch.no_grad():
            for input, target, input_mark, target_mark in valid_data_loader:
                ...
                outputs = outputs[:, -config.horizon :, :series_dim]
            ...
            for i, (input, target, input_mark, target_mark) in enumerate(
                ...
                if config.use_amp == 1:
                    with torch.cuda.amp.autocast():
                        ...
                        outputs, moe_loss = self.model(input)

                        outputs = outputs[:, -config.horizon :, :series_dim]
                        batch_y = batch_y[:, -config.horizon :, :series_dim].to(device)
                    ...
                    loss = criterion(outputs, target) + moe_loss
