...
class MultiheadAttention(nn.Module):
    def __init__(self, configs, d_model, n_heads=1, attn_dropout=0., proj_dropout=0.2):
        ...
        self.attnLinear = configs.Leddam_attnLinear

        if self.attnLinear:
            print('AttnLinear is used...')
            ...
            print('MultiheadAttention is used...')
    ...
    def forward(self, Q: Tensor, K: Optional[Tensor] = None, V: Optional[Tensor] = None, prev: Optional[Tensor] = None,
        ...
        if self.attnLinear:
            # Q: b, n, d
            values = self.v_proj(Q)
            A = F.softplus(self.weight_mat)
            A = F.normalize(A, p=1, dim=-1)
            A = self.dropout(A)

            new_x = A @ values
            output = self.dropout_proj(self.out_proj(new_x))
...
class ScaledDotProductAttention(nn.Module):
    def __init__(self, configs, d_model, n_heads, attn_dropout=0.):
        ...
        self.save_folder = os.path.join('./attn_results',
                                        f'{configs.plot_mat_label}_{configs.seq_len}_{configs.pred_len}')
