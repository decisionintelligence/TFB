...
class TransformerAdapter(ModelBase):
    def __init__(self, model_name, model_class, **kwargs):
        ...
        self.MLP = MLP(input_size=10, hidden_size1=512,output_size=2)
    ...
    def validate(
        ...
        for input, target in valid_data_loader:
            ...
            output = self.model(input, None, dec_input, None)

            output = output[:, -config.horizon:, :series_dim]
            exog_future = target[:, -config.horizon:, series_dim:]
            output = self.MLP(torch.cat((output, exog_future), dim=-1))
    ...
    def forecast_fit(
        ...
        **kwargs,
        ...
        :param train_valid_data: Time series data used for training.【t,c,n】
        ...
        if config.norm:
            ...
            scaled_data = self.scaler.transform(
                train_data.reshape(-1,train_data.shape[-2])
            )
            # 还原回原始三维形状
        ...
        if train_ratio_in_tv != 1:
            if config.norm:
                # 转换时先展平为二维
                scaled_data = self.scaler.transform(
                    valid_data.reshape(-1,valid_data.shape[-2])
                )
                # 还原回原始三维形状
        ...
        for epoch in range(config.num_epochs):
            ...
            for i, (input, target) in enumerate(
                ...
                # transformer_loss = criterion(transformer_output[:, -config.horizon:, :series_dim], target)
                loss = criterion(output, target)
                # print(f"Epoch {epoch}: Transformer Loss: {transformer_loss.item()}, MLP Loss: {loss.item()}")
    ...
    def batch_forecast(
        ...
        answers = torch.tensor(self._perform_rolling_predictions(horizon, input_np, None, device))
        answers = answers[:, -horizon:, :series_dim].to(device)
        output = torch.tensor(answers[:, -horizon:, :series_dim]).to(device)
        exog_future = torch.tensor(exog_futures[i*real_batch_size:(i+1)*real_batch_size, -horizon:,:]).to(device)
        answers = self.MLP(torch.cat((
            output.to(torch.float32),  # 强制转换类型
    ...
    def _perform_rolling_predictions(
        ...
        """
        rolling_time = 0
        input_np, target_np = self._get_rolling_data(
        ...
        with torch.no_grad():
            ...
            while not answers or sum(a.shape[1] for a in answers) < horizon:
                ...
                (
                    ...
                    # input_mark_np,
                    # target_mark_np,
                ) = self._get_rolling_data(input_np, output, None, rolling_time)
