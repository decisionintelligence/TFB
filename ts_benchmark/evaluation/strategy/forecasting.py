...
class ForecastingStrategy(Strategy, metaclass=abc.ABCMeta):
    ...
    def execute(self, series_list: list, model_factory: ModelFactory) -> Any:
        """
        ...
        deterministic_mode = self._get_scalar_config_value("deterministic", series_list[0])
        seed = self._get_scalar_config_value("seed", series_list[0])
        ...
        meta_info = data_pool.get_series_meta_info(series_list[0])

        data = np.stack([data_pool.get_series(series_name).to_numpy() for series_name in series_list], axis=2)

        try:
            single_series_results = self._execute(
                data, meta_info, model_factory, series_list[0]
            )
